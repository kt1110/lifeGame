...
public class GameFrame
{
	...
	int multipyCycle=1;//繁殖周期
	int multipyCount=0;//繁殖次数
	int row=20;//地图大小
	int col=20;
	int[][] cells= new int[20][20];//细胞阵列
	...
	//按钮
	JButton changeButton;//生成地图
	JButton randomButton;//随机生成细胞
	JButton startButton;//开始繁殖
	JButton stopButton;//停止繁殖
	...
	public GameFrame()
	{
		...
		//获取细胞世界
		cells=gameControl.getAllCellState();
		//初始化面板
		operatePanel=getOperatePanel();//控制面板
		viewPanel=getViewPanel();//显示面板
		//面板添加到框架内
...
class MultipyThread extends Thread
{
	...
	public void run()
	{
		...
		while(isRunning)
		{
			...
			cells=gameControl.getAllCellState();//获取改变后细胞生存数组
...
class ClickMonitor implements MouseListener
{
	...
	public void mouseClicked(MouseEvent e) {
		...
		boolean beforeStatus=gameControl.changeCellState(row, col);
		if(beforeStatus)
		{
